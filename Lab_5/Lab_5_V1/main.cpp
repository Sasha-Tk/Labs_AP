...
Date SearchMaxData(char* c_date, int n, Date* D, ...);
...
int main(void) {
	...
	gets_s(c_date);
	max = SearchMaxData(c_date, N, dates, (dates + 1), (dates + 2), (dates + 3));
...
Date SearchMaxData(char* c_date, int n, Date* D, ...)
{
	...
	char* max_in_str;
	max_s = **p;
	max_in_str = SearchMaxStrDate(c_date, (*p)->dates, (*p)->k);
	DiffDates(max_in_str, c_date, m_date);
	for (int i = 1; i < n; i++) {
		max_in_str = SearchMaxStrDate(c_date, (*(p + i))->dates, (*(p + i))->k);
