...
Date SearchMaxData_V2(char* c_date, const char* format, ...);
...
int main(void) {
	...
	gets_s(c_date);
	max = SearchMaxData_V2(c_date, "P P P D", dates, (dates + 1), (dates + 2), *(dates + 3));
...
Date SearchMaxData_V2(char* c_date, const char* format, ...)
{
	const char* p_f = format;
	const char** p = &format;
	char* max_in_str;
	int date[3] = { 0 };
	int m_date[3] = { 0 };
	Date max_s = { "",0, -1 };
	while (*p_f != 'D' && *p_f != 'P' && *p_f != '\0')	p_f++;
	switch (*p_f++)
	{
	case 'D': {
		p++;
		max_in_str = SearchMaxStrDate(c_date, ((Date*)p)->dates, ((Date*)p)->k);
		DiffDates(max_in_str, c_date, m_date);
		max_s = *(Date*)p;
		p = (const char**)((Date*)p + 1);
		break;
	}
	case 'P': {
		p++;
